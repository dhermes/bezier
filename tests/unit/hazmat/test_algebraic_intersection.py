...
class Test__evaluate3(unittest.TestCase):
    ...
    def _call_function_under_test(nodes, x_val, y_val):
        ...
        return algebraic_intersection._evaluate3(nodes, x_val, y_val)
    ...
    def test_it(self):
        ...
        nodes = np.asfortranarray([[0.0, 1.0, 2.0, 3.0], [0.0, 5.0, 1.0, 5.0]])
        xy_vals = utils.get_random_nodes(shape=(50, 2), seed=81390, num_bits=8)
...
class Test_evaluate(unittest.TestCase):
    ...
    def _call_function_under_test(nodes, x_val, y_val):
        ...
        return algebraic_intersection.evaluate(nodes, x_val, y_val)
    ...
    def test_linear(self):
        # f(x, y) = -4 x + y + 3
        nodes = np.asfortranarray([[1.0, 2.0], [1.0, 5.0]])
        ...
        # f(x, y) = (-12 x + 8 y - 5) / 32
        nodes = np.asfortranarray([[0.0, 0.25], [0.625, 1.0]])
        result0 = self._call_function_under_test(nodes, 0.0, 0.0)
        ...
        # f(x, y) = -x
        nodes = np.asfortranarray([[0.0, 0.0], [0.0, 1.0]])
    ...
    def test_quadratic(self):
        ...
        # f(x, y) = (x - y)^2 - y
        nodes = np.asfortranarray([[0.75, -0.25, -0.25], [0.25, -0.25, 0.25]])
    ...
    def test_cubic(self):
        ...
        nodes = np.asfortranarray(
            [[6.0, -2.0, -2.0, 6.0], [-3.0, 3.0, -3.0, 3.0]]
        )
        xy_vals = utils.get_random_nodes(
...
class Test_eval_intersection_polynomial(unittest.TestCase):
    ...
    def _call_function_under_test(nodes1, nodes2, t):
        ...
        return algebraic_intersection.eval_intersection_polynomial(
            nodes1, nodes2, t
        )

    def test_degrees_1_1(self):
        # f1(x, y) = (8 y - 3) / 8
        nodes1 = np.asfortranarray([[0.0, 1.0], [0.375, 0.375]])
        # x2(t), y2(t) = 1 / 2, 3 s / 4
    ...
    def test_degrees_1_2(self):
        # f1(x, y) = 2 (4 x + 3 y - 24)
        nodes1 = np.asfortranarray([[0.0, 6.0], [8.0, 0.0]])
        # x2(t), y2(t) = 9 t, 18 t (1 - t)
        nodes2 = np.asfortranarray([[0.0, 4.5, 9.0], [0.0, 9.0, 0.0]])
        values = [
            self._call_function_under_test(nodes1, nodes2, 0.0),
...
class Test__to_power_basis11(utils.NumPyTestCase):
    ...
    def _call_function_under_test(nodes1, nodes2):
        ...
        return algebraic_intersection._to_power_basis11(nodes1, nodes2)

    def test_it(self):
        # f1(x, y) = -(12 x - 8 y + 5) / 32
        nodes1 = np.asfortranarray([[0.0, 0.25], [0.625, 1.0]])
        # x2(t), y2(t) = (2 - 3 t) / 4, (3 t + 2) / 4
        nodes2 = np.asfortranarray([[0.5, -0.25], [0.5, 1.25]])
...
class Test__to_power_basis12(utils.NumPyTestCase):
    ...
    def _call_function_under_test(nodes1, nodes2):
        ...
        return algebraic_intersection._to_power_basis12(nodes1, nodes2)

    def test_it(self):
        # f1(x, y) = (2 y - 1) / 2
        nodes1 = np.asfortranarray([[0.0, 1.0], [0.5, 0.5]])
        # x2(t), y2(t) = t, 2 t (1 - t)
        nodes2 = np.asfortranarray([[0.0, 0.5, 1.0], [0.0, 1.0, 0.0]])
...
class Test__to_power_basis13(utils.NumPyTestCase):
    ...
    def _call_function_under_test(nodes1, nodes2):
        ...
        return algebraic_intersection._to_power_basis13(nodes1, nodes2)

    def test_it(self):
        # f1(x, y) = -(152 x + 112 y - 967) / 64
        nodes1 = np.asfortranarray([[2.5625, 0.8125], [5.15625, 7.53125]])
        # x2(t), y2(t) = 3 (14 t + 1) / 8, 18 t^3 - 27 t^2 + 3 t + 7
        nodes2 = np.asfortranarray(
            [[0.375, 2.125, 3.875, 5.625], [7.0, 8.0, 0.0, 1.0]]
        )
        # f1(x2(t), y2(t)) = -63 (t - 1) (4 t - 1)^2 / 32
        result = self._call_function_under_test(nodes1, nodes2)
        # The 1-3 method avoids a division by 3.0
        expected = (
            3.0 * (-63.0 / 32.0) * np.asfortranarray([-1.0, 9.0, -24.0, 16.0])
        )
        self.assertEqual(result, expected)
...
class Test__to_power_basis_degree4(utils.NumPyTestCase):
    ...
    def _call_function_under_test(nodes1, nodes2):
        ...
        return algebraic_intersection._to_power_basis_degree4(nodes1, nodes2)

    def test_degrees_2_2(self):
        # f1(x, y) = (x^2 - 4 x y + 4 y^2 - y) / 16
        nodes1 = np.asfortranarray(
            [[0.375, -0.125, -0.125], [0.0625, -0.0625, 0.0625]]
        )
        # x2(t), y2(t) = (2 t - 3) (2 t - 1) / 4, (2 t - 1)^2 / 4
        nodes2 = np.asfortranarray([[0.75, -0.25, -0.25], [0.25, -0.25, 0.25]])
    ...
    def test_degrees_1_4(self):
        # f1(x, y) = 4 (y - 3)
        nodes1 = np.asfortranarray([[0.0, 4.0], [3.0, 3.0]])
        # x2(t), y2(t) = 4 s, 2 s (s - 1) (5 s^2 - 5 s - 2)
        nodes2 = np.asfortranarray(
            [[0.0, 1.0, 2.0, 3.0, 4.0], [0.0, 1.0, 3.0, 1.0, 0.0]]
        )
...
class Test__to_power_basis23(utils.NumPyTestCase):
    ...
    def _call_function_under_test(nodes1, nodes2):
        ...
        return algebraic_intersection._to_power_basis23(nodes1, nodes2)

    def test_it(self):
        # f1(x, y) = 4 (4 x^2 - 12 x - 4 y + 11)
        nodes1 = np.asfortranarray([[0.5, 1.5, 2.5], [1.5, -0.5, 1.5]])
...
class Test__to_power_basis_degree8(utils.NumPyTestCase):
    ...
    def _call_function_under_test(nodes1, nodes2):
        ...
        return algebraic_intersection._to_power_basis_degree8(nodes1, nodes2)

    def test_degrees_2_4(self):
        # f1(x, y) = 2 (9 x - 2 y^2 - 6 y)
        nodes1 = np.asfortranarray([[0.0, 1.0, 4.0], [0.0, 1.5, 3.0]])
        # x2(t), y2(t) = 4 t, -t (7 t^3 - 6 t - 4)
        nodes2 = np.asfortranarray(
            [[0.0, 1.0, 2.0, 3.0, 4.0], [0.0, 1.0, 3.0, 6.0, 3.0]]
        )
        # f1(x2(t), y2(t)) = (
...
class Test__to_power_basis33(utils.NumPyTestCase):
    ...
    def _call_function_under_test(nodes1, nodes2):
        ...
        return algebraic_intersection._to_power_basis33(nodes1, nodes2)

    def test_it(self):
        # f1(x, y) = x^3 - 9 x^2 + 27 x - 27 y
        nodes1 = np.asfortranarray(
            [[0.0, 1.0, 2.0, 3.0], [0.0, 1.0, 1.0, 1.0]]
        )
        # x2(t), y2(t) = (1 - t) (t^2 + t + 1), 3 (1 - t)
        nodes2 = np.asfortranarray(
            [[1.0, 1.0, 1.0, 0.0], [3.0, 2.0, 1.0, 0.0]]
        )
        # f1(x2(t), y2(t)) = (
...
class Test_to_power_basis(utils.NumPyTestCase):
    ...
    def _call_function_under_test(nodes1, nodes2):
        ...
        return algebraic_intersection.to_power_basis(nodes1, nodes2)

    def test_degrees_1_1(self):
        # f1(x, y) = -x
        nodes1 = np.asfortranarray([[0.0, 0.0], [0.0, 1.0]])
        # x2(t), y2(t) = 1, t
        nodes2 = np.asfortranarray([[1.0, 1.0], [0.0, 1.0]])
        # f1(x2(t), y2(t)) = -1
        result = self._call_function_under_test(nodes1, nodes2)
    ...
    def test_degrees_1_3(self):
        # f1(x, y) = -(2 x - y - 1) / 2
        nodes1 = np.asfortranarray([[0.5, 1.0], [0.0, 1.0]])
        # x2(t), y2(t) = -t (2 t^2 - 3 t - 3) / 4, -(3 t^3 - 3 t - 1) / 2
        nodes2 = np.asfortranarray(
            [[0.0, 0.25, 0.75, 1.0], [0.5, 1.0, 1.5, 0.5]]
        )
    ...
    def test_degrees_1_4(self):
        # f1(x, y) = 4 y - 3 x
        nodes1 = np.asfortranarray([[0.0, 4.0], [0.0, 3.0]])
        # x2(t), y2(t) = 4 t, -t (7 t^3 - 6 t - 4)
        nodes2 = np.asfortranarray(
            [[0.0, 1.0, 2.0, 3.0, 4.0], [0.0, 1.0, 3.0, 6.0, 3.0]]
        )
    ...
    def test_degrees_2_2(self):
        ...
        nodes1 = np.asfortranarray([[1.0, 0.75, 0.0], [0.0, 0.75, 1.0]])
        # x2(t), y2(t) = (t - 1) (5 t - 8) / 8, -(t - 1) (7 t + 8) / 8
        nodes2 = np.asfortranarray([[1.0, 0.1875, 0.0], [1.0, 0.9375, 0.0]])
    ...
    def test_degrees_2_3(self):
        ...
        nodes1 = np.asfortranarray([[0.25, 0.625, 1.0], [0.625, 0.25, 1.0]])
        # x2(t), y2(t) = -t (2 t^2 - 3 t - 3) / 4, -(3 t^3 - 3 t - 1) / 2
        nodes2 = np.asfortranarray(
            [[0.0, 0.25, 0.75, 1.0], [0.5, 1.0, 1.5, 0.5]]
        )
    ...
    def test_degrees_3_3(self):
        ...
        nodes1 = np.asfortranarray(
            [[0.0, 0.375, 0.625, 1.0], [1.0, -1.0, 0.0, 1.0]]
        )
        # x2(t), y2(t) = -t (2 t^2 - 3 t - 3) / 4, -(3 t^3 - 3 t - 1) / 2
        nodes2 = np.asfortranarray(
            [[0.0, 0.25, 0.75, 1.0], [0.5, 1.0, 1.5, 0.5]]
...
class Test_polynomial_norm(unittest.TestCase):
    ...
    def _call_function_under_test(coeffs):
        ...
        return algebraic_intersection.polynomial_norm(coeffs)
...
class Test_roots_in_unit_interval(utils.NumPyTestCase):
    ...
    def _call_function_under_test(coeffs):
        ...
        return algebraic_intersection.roots_in_unit_interval(coeffs)
    ...
    def test_it(self):
        ...
        for index in (0, 1, 2):
            ...
            self.assertAlmostEqual(all_roots[index], expected, delta=LOCAL_EPS)
...
class Test__strip_leading_zeros(utils.NumPyTestCase):
    ...
    def _call_function_under_test(coeffs, **kwargs):
        ...
        return algebraic_intersection._strip_leading_zeros(coeffs, **kwargs)
...
class Test__check_non_simple(utils.NumPyTestCase):
    ...
    def _call_function_under_test(coeffs):
        ...
        return algebraic_intersection._check_non_simple(coeffs)
...
class Test__resolve_and_add(utils.NumPyTestCase):
    ...
    def _call_function_under_test(
        ...
        return algebraic_intersection._resolve_and_add(
            nodes1, s_val, final_s, nodes2, t_val, final_t
        )
...
class Test_intersect_curves(utils.NumPyTestCase):
    ...
    def _call_function_under_test(nodes1, nodes2):
        ...
        return algebraic_intersection.intersect_curves(nodes1, nodes2)
    ...
    def test_coincident(self):
        ...
        nodes1 = np.asfortranarray([[0.0, 1.0, 4.0], [0.0, 2.0, 0.0]])
        # x2(t), y2(t) = 3 (t + 1) (3 t + 7) / 8, -3 (t + 1) (3 t - 1) / 4
        nodes2 = np.asfortranarray([[2.625, 4.5, 7.5], [0.75, 0.0, -3.0]])
        # f1(x2(t), y2(t)) = 0
        with self.assertRaises(NotImplementedError):
...
class Test_normalize_polynomial(utils.NumPyTestCase):
    ...
    def _call_function_under_test(coeffs, **kwargs):
        ...
        return algebraic_intersection.normalize_polynomial(coeffs, **kwargs)
...
class Test__get_sigma_coeffs(utils.NumPyTestCase):
    ...
    def _call_function_under_test(coeffs):
        ...
        return algebraic_intersection._get_sigma_coeffs(coeffs)
...
class Test_bernstein_companion(utils.NumPyTestCase):
    ...
    def _call_function_under_test(coeffs):
        ...
        return algebraic_intersection.bernstein_companion(coeffs)
    ...
    def test_quadratic(self):
        ...
        expected = np.asfortranarray([[-0.5, 0.0], [1.0, 0.0]])
...
class Test_bezier_roots(utils.NumPyTestCase):
    ...
    def _call_function_under_test(coeffs):
        ...
        return algebraic_intersection.bezier_roots(coeffs)
...
class Test_lu_companion(utils.NumPyTestCase):
    ...
    def _call_function_under_test(top_row, value):
        ...
        return algebraic_intersection.lu_companion(top_row, value)
    ...
    def _check_lu(self, lu_mat, expected_a):
        ...
        a_mat = helpers.matrix_product(l_mat, u_mat)
    ...
    def test_quadratic(self):
        ...
        expected = np.asfortranarray([[1.0, -value], [-7.0, -10.0]])
        self.assertEqual(expected, lu_mat)
        self.assertEqual(one_norm, 8.0)
        expected_a = np.asfortranarray([[1.0, -value], [-7.0, 4.0]])
    ...
    def test_cubic(self):
        ...
        lu_mat, one_norm = self._call_function_under_test(top_row, value)
        expected = np.asfortranarray(
            [[1.0, -value, 0.0], [0.0, 1.0, -value], [-6.0, -17.0, -53.0]]
        )
        self.assertEqual(expected, lu_mat)
        self.assertEqual(one_norm, 7.0)
        expected_a = np.asfortranarray(
            [[1.0, -value, 0.0], [0.0, 1.0, -value], [-6.0, 1.0, -2.0]]
        )
        self._check_lu(lu_mat, expected_a)
...
class Test__reciprocal_condition_number(utils.NumPyTestCase):
    ...
    def _call_function_under_test(lu_mat, one_norm):
        ...
        return algebraic_intersection._reciprocal_condition_number(
            lu_mat, one_norm
        )
    ...
    def test_singular(self):
        ...
        #      [ 1.   1.5]]
        one_norm = 4.5
        lu_mat = np.asfortranarray([[2.0, 3.0], [0.5, 0.0]])
    ...
    def test_invertible(self):
        ...
        lu_mat = np.asfortranarray(
            [[4.0, 10.0, 0.0], [0.0, 16.0, 32.0], [0.5, -0.5, 16.0]]
        )
        rcond = self._call_function_under_test(lu_mat, one_norm)
...
class Test_bezier_value_check(utils.NumPyTestCase):
    ...
    def _call_function_under_test(coeffs, s_val, **kwargs):
        ...
        return algebraic_intersection.bezier_value_check(
            coeffs, s_val, **kwargs
        )
    ...
    def test_quadratic(self):
        ...
        check_values = [
            self._call_function_under_test(coeffs, s_val) for s_val in s_vals
        ]
        self.assertEqual(
...
class Test_poly_to_power_basis(utils.NumPyTestCase):
    ...
    def _call_function_under_test(bezier_coeffs):
        ...
        return algebraic_intersection.poly_to_power_basis(bezier_coeffs)
...
class Test_all_intersections(utils.NumPyTestCase):
    ...
    def _call_function_under_test(nodes_first, nodes_second):
        ...
        return algebraic_intersection.all_intersections(
