...
-  Integrating ``black`` `code formatter`_ into the ``nox --session lint``
   session to ensure consistent formatting
...
-  Added ``nox --session validate_functional_test_cases`` session to ensure
   that functional test cases always adhere to the JSON schema.
