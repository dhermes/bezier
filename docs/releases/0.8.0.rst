...
-  Removed ``BAD_TANGENT`` status enum
   ...
   `function <https://bezier.readthedocs.io/en/0.8.0/algorithm-helpers.html#bezier._surface_helpers.classify_intersection>`__
   for an example.)
...
-  Removed ``PARALLEL`` status enum
   ...
   (if they do) using a modified Newton iteration to converge to a root.
...
-  Added custom ``UnsupportedDegree``
   `exception <https://bezier.readthedocs.io/en/0.8.0/reference/bezier.html#bezier.UnsupportedDegree>`__
